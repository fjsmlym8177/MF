...
namespace MF.Core.Pipeline
{
    public class RedisPipelineContext : IPipelineContext
    {
        ...
        public void AddRange<T>(IList<T> list, Func<T, string> pipelineName)
        {
            ...
            {
                tasks.Add(batch.ListRightPushAsync(pipelineName(item), item.ToJson()));
            }
        ...
        public void ExecutePipeline<T>(IList<string> pipelineNames, Func<IList<T>, string> func)
        {
                ...
                Task.Run(() =>
                {
                    ...
                    while (true)
                    {
                        ...
                        foreach (var task in tasks)
                        {
                            ...
                            {
                                list.Add(task.Result.ToString().ToDeserialize<T>());
                            }
                        }
